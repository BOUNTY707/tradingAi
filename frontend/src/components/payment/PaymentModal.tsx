...
import { Check, X, Upload, CheckCircle, AlertCircle, Copy } from 'lucide-react'
...
function BankCardLogo() {
  return (
    <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
      <rect width="52" height="52" rx="14" fill="url(#pm_card_bg)"/>
      ...
      <defs>
        <linearGradient id="pm_card_bg" x1="0" y1="0" x2="52" y2="52" gradientUnits="userSpaceOnUse">
...
const METHODS = [
  ...
  { id: 'uzum',  name: 'Uzum Bank',  sub: 'Karta orqali',  glow: '#FF7A29', cardNum: '4916 9903 1372 1554', Logo: () => <ImgLogo src={uzumIcon}  alt="Uzum"  /> },
  { id: 'card',  name: 'Bank Karta', sub: 'Istalgan bank', glow: '#6D28D9', cardNum: '9860 0101 2689 9433', Logo: BankCardLogo },
]

export default function PaymentModal({ plan, onClose }: { plan: PaymentPlan; onClose: () => void }) {
  ...
  if (done) return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-5">
      ...
      <motion.div initial={{ scale: 0.85, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }}
        ...
        <p className="text-white/50 text-sm leading-relaxed mb-1">Screenshotingiz qabul qilindi.</p>
        <p className="text-white/70 text-sm mb-6"><span className="text-green-400 font-semibold">1–2 soat</span> ichida hisobingiz faollashtiriladi.</p>
        <div className="text-xs text-white/30 mb-6 font-mono bg-white/3 rounded-xl px-4 py-2.5 border border-white/5">mehrobakooo@gmail.com</div>
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        style={{ maxHeight: 'calc(100dvh - 40px)', background: 'linear-gradient(160deg,rgba(9,9,16,0.99),rgba(13,9,22,0.99))', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 24, boxShadow: '0 24px 80px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.04) inset' }}>
        ...
        <div className="px-6 pt-5 pb-4 shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-start justify-between">
            ...
            <button onClick={onClose} className="w-9 h-9 rounded-2xl flex items-center justify-center transition-colors hover:bg-white/8" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto overscroll-contain px-6 py-5 space-y-5" style={{ scrollbarWidth: 'none' }}>
          {/* Method selection */}
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {METHODS.map(m => {
                ...
                return (
                  <motion.button key={m.id} onClick={() => setMethod(m)} whileTap={{ scale: 0.97 }}
                    className="relative p-4 rounded-2xl text-left transition-all flex items-center gap-3.5 overflow-hidden"
                    style={{ background: active ? `linear-gradient(135deg,${m.glow}22,${m.glow}08)` : 'rgba(255,255,255,0.03)', border: `1.5px solid ${active ? m.glow+'55' : 'rgba(255,255,255,0.07)'}`, boxShadow: active ? `0 0 24px ${m.glow}20,inset 0 1px 0 ${m.glow}30` : 'none' }}>
                    {active && <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(circle at 20% 50%,${m.glow}18 0%,transparent 70%)` }} />}
                    <div className="relative shrink-0" style={{ filter: active ? `drop-shadow(0 4px 12px ${m.glow}60)` : 'none', transition: 'filter .25s' }}><m.Logo /></div>
                    <div className="relative min-w-0">
                      <p className="font-semibold text-sm leading-tight truncate" style={{ color: active ? 'white' : 'rgba(255,255,255,0.75)' }}>{m.name}</p>
                      <p className="text-xs mt-0.5 truncate" style={{ color: active ? m.glow : 'rgba(255,255,255,0.3)' }}>{m.sub}</p>
                    </div>
                    {active && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 400 }} className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: m.glow }}><Check size={11} className="text-white" strokeWidth={3} /></motion.div>}
                  </motion.button>
                )
              })}
            </div>
          ...
          <AnimatePresence mode="wait">
            {method && (
              <motion.div key={method.id} initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.22 }} className="rounded-2xl overflow-hidden" style={{ background: `linear-gradient(145deg,${method.glow}16,${method.glow}05,rgba(0,0,0,0.3))`, border: `1px solid ${method.glow}35`, boxShadow: `0 8px 32px ${method.glow}12` }}>
                <div className="flex items-center gap-4 px-5 py-4" style={{ borderBottom: `1px solid ${method.glow}20` }}>
                  <div style={{ filter: `drop-shadow(0 4px 16px ${method.glow}80)` }}><method.Logo /></div>
                  <div>
                ...
                <div className="px-5 py-4" style={{ borderBottom: `1px solid ${method.glow}15` }}>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-1 flex-wrap">
                      {method.cardNum.split(' ').map((g, i) => <span key={i} className="font-mono text-white font-bold text-xl tracking-widest">{g}</span>)}
                    </div>
                    <motion.button onClick={copy} whileTap={{ scale: 0.9 }} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all shrink-0" style={{ background: copied ? 'rgba(0,209,98,0.18)' : `${method.glow}18`, color: copied ? '#00d162' : method.glow, border: `1px solid ${copied ? 'rgba(0,209,98,0.4)' : method.glow+'40'}` }}>
                ...
                <div className="mx-5 my-4 flex items-start gap-2.5 rounded-xl px-4 py-3" style={{ background: 'rgba(234,179,8,0.07)', border: '1px solid rgba(234,179,8,0.18)' }}>
                  <AlertCircle size={14} className="text-yellow-400 shrink-0 mt-0.5" />
                  <p className="text-xs text-yellow-300/75 leading-relaxed">Izohga <strong className="text-yellow-200">TradeAI {plan.duration}</strong> va ro'yxatdan o'tgan telefon raqamingizni yozing</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                {!preview ? (
                  <motion.div
                    ...
                    style={{ border: `2px dashed ${dragging ? method.glow+'cc' : 'rgba(255,255,255,0.09)'}`, background: dragging ? `${method.glow}08` : 'rgba(255,255,255,0.02)' }}>
                    <div className="flex flex-col items-center py-8 gap-3">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: `${method.glow}15`, border: `1px solid ${method.glow}30` }}>
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl overflow-hidden relative" style={{ border: `1px solid ${method.glow}40`, boxShadow: `0 4px 24px ${method.glow}18` }}>
                    ...
                    <button onClick={() => { setFile(null); setPreview(null) }} className="absolute top-3 right-3 w-7 h-7 rounded-xl flex items-center justify-center transition-colors hover:bg-black/80" style={{ background: 'rgba(0,0,0,0.65)', border: '1px solid rgba(255,255,255,0.12)' }}>
                      <X size={13} className="text-white" />
                    </button>
                    <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                      <div className="flex items-center gap-1.5"><CheckCircle size={13} className="text-green-400" /><span className="text-xs text-green-400 font-semibold">Yuklandi</span></div>
        ...
        {/* Footer */}
        <div className="shrink-0 px-6 py-5 space-y-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)', background: 'rgba(5,5,8,0.9)', borderRadius: '0 0 24px 24px' }}>
          <Button onClick={() => { if (method && file) setDone(true) }} disabled={!method || !file} size="lg" className="w-full !rounded-2xl !py-4 !text-base font-bold">
            <CheckCircle size={18} /> Tasdiqlashni yuborish
          </Button>
          <p className="text-center text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>To'lov tekshirilgandan so'ng 1–2 soat ichida faollashtiriladi</p>
        </div>
      </motion.div>
    </div>
  )
